...
class JiangsuRuleEngine(BaseRuleEngine):

    def __init__(self):
        self.config = PROVINCE_CONFIGS["jiangsu"]
    ...
    def validate_subjects(self, student: StudentProfile) -> bool:
        if not student.subject_category or student.subject_category not in self.config["subject_categories"]:
            return False
    ...
    def check_subject_match(self, student: StudentProfile, plan: CurrentYearPlan) -> EligibilityResult:
        if plan.subject_category and plan.subject_category != student.subject_category:
            return EligibilityResult(
                False,
                f"首选科目不匹配，要求{plan.subject_category}，考生选择{student.subject_category}"
            )

        required = plan.subject_requirements
        if not required:
            return EligibilityResult(True, "该专业组不限再选科目")

        student_subjects = set(student.subjects)
        required_set = set(required)
        if student_subjects & required_set:
            return EligibilityResult(True, f"再选科目匹配: {', '.join(student_subjects & required_set)}")
        return EligibilityResult(
            False,
            f"再选科目不匹配，要求{required}中至少一门"
        )
    ...
    def check_eligibility(self, student: StudentProfile, plan: CurrentYearPlan) -> EligibilityResult:
        ...
        if student.budget_limit and plan.tuition and plan.tuition > student.budget_limit:
            return EligibilityResult(False, f"学费超出预算")
    ...
    def check_gradient(self, tier_counts: dict[str, int]) -> list[str]:
        ...
        if tier_counts.get("backup", 0) < 2:
            warnings.append("垫底志愿少于2个")
        return warnings
